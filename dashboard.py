...
def calcular_kpis(df):
    ...
    try:
        ...
        pct_riesgo = (
            round(100 * (df["Categoria_Riesgo"] == "Alto").sum() / total, 1)
            if total > 0
            else 0
        )
        pct_reprobado = (
            round(100 * (df["Grade"] == "F").sum() / total, 1) if total > 0 else 0
        )
...
def mostrar_kpis(
    ...
    col2.metric(
        ...
        delta=f"{promedio - promedio_base:+.1f} pts vs total"
        if total != total_base
        else None,
    )
    ...
    delta_riesgo = (
        round(pct_riesgo - pct_riesgo_base, 1) if total != total_base else None
    )
    col3.metric(
    ...
    delta_repro = (
        round(pct_reprobado - pct_reprobado_base, 1) if total != total_base else None
    )
    col4.metric(
...
def grafico_grades_bar(df):
    ...
    try:
        ...
        fig, ax = plt.subplots(figsize=(5, 4))
        bars = ax.bar(
            conteo.index, conteo.values, color=colores, edgecolor="white", linewidth=1.5
        )
...
def grafico_departamento(df):
    ...
    try:
        resumen = (
            df.groupby("Department", observed=True)["Total_Score"]
            .mean()
        ...
        bars = ax.barh(
            resumen.index, resumen.values, color=colores, edgecolor="white", linewidth=1
        )
...
def grafico_estres_reprobacion(df):
    ...
    try:
        resumen = (
            df.groupby("Stress_Level (1-10)", observed=True)["Grade"]
            .apply(lambda s: 100 * (s == "F").mean())
        ...
        colores = [
            "#2ecc71" if t < 15 else "#f39c12" if t < 20 else "#e74c3c"
            for t in resumen["Tasa"]
        ]

        fig, ax = plt.subplots(figsize=(7, 4))
        bars = ax.bar(
            resumen["Nivel_Estres"],
            resumen["Tasa"],
            color=colores,
            edgecolor="white",
            linewidth=1.5,
...
def grafico_brecha_internet(df):
    ...
    try:
        resumen = (
            df.groupby(
                ["Family_Income_Level", "Internet_Access_at_Home"], observed=True
            )["Total_Score"]
            .mean()
            .round(2)
            .unstack()
...
def grafico_eficiencia(df):
    ...
    try:
        resumen = (
            df.groupby("Categoria_Riesgo", observed=True)
            .agg(
                Eficiencia=("Eficiencia_Estudio", "mean"),
        ...
        bars1 = ax.bar(
            ...
            resumen["Eficiencia"],
            width,
            label="Eficiencia Promedio (pts/hora)",
            color="#2ecc71",
            edgecolor="white",
        )
        bars2 = ax.bar(
            ...
            resumen["Horas"],
            width,
            label="Horas Promedio por Semana",
            color="#2980b9",
            edgecolor="white",
        )
        ...
        ax.set_title(
            "Eficiencia y Horas de Estudio por Categoría de Riesgo", fontweight="bold"
        )
...
def construir_sidebar(df):
    ...
    try:
        ...
        if "filtro_dept" not in st.session_state:
            st.session_state.filtro_dept = depts

        if "filtro_riesgo" not in st.session_state:
            st.session_state.filtro_riesgo = riesgos

        if "filtro_ingreso" not in st.session_state:
            st.session_state.filtro_ingreso = ingresos

        if "filtro_grade" not in st.session_state:
            st.session_state.filtro_grade = grades

        if "filtro_internet" not in st.session_state:
            st.session_state.filtro_internet = "Todos"

        if "filtro_asistencia" not in st.session_state:
            st.session_state.filtro_asistencia = 0
        ...
        rango_asistencia = st.sidebar.slider(
            ...
            min_value=0,
            max_value=100,
            key="filtro_asistencia",
            help="Filtrá estudiantes con al menos este % de asistencia",
        )

        st.sidebar.markdown("---")
        st.sidebar.markdown(
            "📌 **Nota:** Los gráficos se actualizan automáticamente al cambiar los filtros."
        )
        ...
        if sel_ingreso:
            df_filtrado = df_filtrado[
                df_filtrado["Family_Income_Level"].isin(sel_ingreso)
            ]
        ...
        if internet == "Con Internet":
            df_filtrado = df_filtrado[
                df_filtrado["Internet_Access_at_Home"].isin(["Yes"])
            ]
        elif internet == "Sin Internet":
            df_filtrado = df_filtrado[
                df_filtrado["Internet_Access_at_Home"].isin(["No"])
            ]

        if rango_asistencia > 0:
            df_filtrado = df_filtrado[
                df_filtrado["Attendance (%)"] >= rango_asistencia
            ]
...
def main():
    ...
    st.markdown(
        "Explorá el rendimiento, riesgo y equidad del dataset de 5.000 estudiantes universitarios."
    )
    st.markdown("---")
    ...
    if error:
        ...
        st.info(
            "Verificá que el archivo `students_clean.csv` esté disponible en el repositorio de GitHub."
        )
        st.stop()
    ...
    if len(df_filtrado) == 0:
        st.warning(
            "⚠️ No hay datos que coincidan con los filtros seleccionados. Ajustá los filtros o usá el botón 🔄 Resetear."
        )
        st.stop()
    ...
    if len(df_filtrado) < len(df):
        st.info(
            f"🔍 Mostrando **{len(df_filtrado):,}** de **{len(df):,}** estudiantes según los filtros aplicados."
        )

    # ── KPIs ─────────────────────────────────────────────────────
    st.subheader("📌 Indicadores Clave")
    st.markdown(
        "Las flechas muestran la variación respecto al dataset completo sin filtros."
    )
    ...
    st.markdown(
        "¿Cómo se distribuyen los estudiantes según su riesgo y calificación final?"
    )
    ...
    with col1:
        ...
        st.info(
            f"📌 **{riesgo_alto_n} estudiantes** están en Riesgo Alto. "
    ...
    with col2:
        ...
        st.info(
            f"📌 **{reprobados_n} estudiantes** obtuvieron F (reprobado). "
    ...
    if fig:
        ...
        plt.close(fig)
    mejor_dept = (
        df_filtrado.groupby("Department", observed=True)["Total_Score"].mean().idxmax()
    )
    peor_dept = (
        df_filtrado.groupby("Department", observed=True)["Total_Score"].mean().idxmin()
    )
    st.info(
        f"📌 **{mejor_dept}** lidera el rendimiento académico. "
    ...
    st.markdown(
        "Análisis dinámico del estrés, el contexto socioeconómico y el rendimiento según los filtros aplicados."
    )
    col1, col2 = st.columns(2)
    ...
    with col1:
        ...
        # Nota dinámica: estrés y reprobación
        resumen_estres = (
            df_filtrado.groupby("Stress_Level (1-10)")["Grade"]
            .apply(lambda s: 100 * (s == "F").mean())
            .reset_index(name="Tasa_Reprobacion")
        )
        if not resumen_estres.empty:
            nivel_mayor_reprobacion = resumen_estres.loc[
                resumen_estres["Tasa_Reprobacion"].idxmax()
            ]

            st.info(
                f"📌 En el grupo filtrado, el nivel de estrés con mayor tasa de reprobación es "
                f"**{int(nivel_mayor_reprobacion['Stress_Level (1-10)'])}**, "
                f"con una tasa de **{nivel_mayor_reprobacion['Tasa_Reprobacion']:.1f}%**."
            )
        else:
            st.info(
                "📌 No hay datos suficientes para analizar la relación entre estrés y reprobación."
            )
    ...
    with col2:
        ...
        # Nota dinámica: ingresos + acceso a internet
        resumen_brecha = (
            df_filtrado.groupby(["Family_Income_Level", "Internet_Access_at_Home"])[
                "Total_Score"
            ]
            .mean()
            .reset_index()
        ...
        if not resumen_brecha.empty:
            peor_grupo = resumen_brecha.loc[resumen_brecha["Total_Score"].idxmin()]

            ingreso_txt = peor_grupo["Family_Income_Level"]
            internet_txt = (
                "con acceso a internet"
                if peor_grupo["Internet_Access_at_Home"] == "Yes"
                else "sin acceso a internet"
            )

            st.info(
                f"📌 En el grupo filtrado, el menor rendimiento corresponde a estudiantes de ingresos "
                f"**{ingreso_txt}** y **{internet_txt}**, "
                f"con un score promedio de **{peor_grupo['Total_Score']:.1f} puntos**."
            )
        else:
            st.info(
                "📌 No hay datos suficientes para analizar la brecha por ingresos e internet."
            )
    ...
    st.markdown("Análisis dinámico de la eficiencia y las horas de estudio según la categoría de riesgo.")
    ...
    if fig:
        ...
        # Nota dinámica: eficiencia y horas de estudio
        resumen_eficiencia = (
            df_filtrado.groupby("Categoria_Riesgo", observed=True)
            .agg(
                Eficiencia=("Eficiencia_Estudio", "mean"),
                Horas=("Study_Hours_per_Week", "mean"),
            )
        ...
        if not resumen_eficiencia.empty:
            menor_eficiencia = resumen_eficiencia.loc[
                resumen_eficiencia["Eficiencia"].idxmin()
            ]

            mayor_horas = resumen_eficiencia.loc[resumen_eficiencia["Horas"].idxmax()]

            st.info(
                f"📌 En el grupo filtrado, la categoría con menor eficiencia de estudio es "
                f"**{menor_eficiencia['Categoria_Riesgo']}**, "
                f"con **{menor_eficiencia['Eficiencia']:.2f} pts/hora**. "
                f"La categoría que más horas estudia en promedio es "
                f"**{mayor_horas['Categoria_Riesgo']}**, "
                f"con **{mayor_horas['Horas']:.1f} horas semanales**."
            )
        else:
            st.info(
                "📌 No hay datos suficientes para analizar la eficiencia de estudio."
            )
